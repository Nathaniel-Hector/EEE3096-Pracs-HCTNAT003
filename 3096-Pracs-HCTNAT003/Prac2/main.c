...
int currentIndex = 0;
int maxIndex = (sizeof(patterns) / sizeof(patterns[0]))-1;
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t currentPattern = read_from_address(currentIndex);
	if (currentPattern == patterns[currentIndex]){
		GPIOB->ODR = currentPattern;
	}
	else{
		GPIOB->ODR = 0b00000001;
	}
//	GPIOB->ODR = currentIndex;
