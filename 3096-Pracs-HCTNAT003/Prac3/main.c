...
int main(void)
{
  ...
  while (1)
  {
	...
	// ADC to LCD; Read POT1 value and write to LCD
	adc_val = pollADC();
	sprintf(adcValueString, "%d", adc_val);
...
void EXTI0_1_IRQHandler(void)
{
	...
	//Frequency of toggles controlled by delay function in main() while
	//loop. Change delay value to change frequency.
...
// Complete the writeLCD function
void writeLCD(char *char_in){
    delay(3000);
	lcd_command(CLEAR);
...
uint32_t pollADC(void){
	...
	//  In order to do ADC conversion by following, the following must be done:
	//	-	Activate ADC peripheral and begin conversion
	//	-	Wait for conversion to complete
	//	-	Retrieve completed conversion result
	//	-	Stop the conversion
	//  We call methods which allow us to do each of these steps with regards to
	//  P01, PA6 (Potentiometer 1)
...
uint32_t ADCtoCCR(uint32_t adc_val){
	// Calculate CCR using an appropriate equation
	uint32_t MAX_ADC_VALUE = 4095;
	uint32_t TIM3_ARR_VALUE = 47999;
	uint32_t ccrValue = ((float)adc_val/MAX_ADC_VALUE) * TIM3_ARR_VALUE;
	return ccrValue;
}
